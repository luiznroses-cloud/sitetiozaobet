...
include_once(__DIR__ . '/bonus.php');
#=====================================================#
# DATA POPUPS
...
function pegarLinkJogoigamewin($provider, $game, $email)
{
    ...
    if (!$data_igamewin || !isset($data_igamewin['agent_code'])) {
        return array(
            'gameURL' => null,
            'error' => 'Configuração iGameWin não encontrada no banco',
            'debug' => $data_igamewin
        );
    }
    ...
    $json_data = json_encode($dataRequest, JSON_UNESCAPED_UNICODE);

    // DEBUG REQUEST
    file_put_contents(
        __DIR__ . '/debug_igamewin.log',
...
function consultarSaldoAgenteIGameWin()
{
    global $data_igamewin;
    ...
    $response = enviarRequest($data_igamewin['url'], $json_data);
    $data = json_decode($response, true);
    if (isset($data['status']) && intval($data['status']) === 1 && isset($data['agent']['balance'])) {
...
function pegarLinkJogoApiPlayFiver($provedor, $game, $email, $saldo)
{
    ...
    $data = array(
        ...
        'game_code'     => $game,
        'game_original' => true,  // Sempre true para jogos originais
    ...
    if (isset($data_response['status']) && $data_response['status'] == 1) {
        ...
        $error_msg = $data_response['msg'] ?? $data_response['message'] ?? 'Erro desconhecido';
        error_log("[PlayFiver] Erro - " . $error_msg);
        $games = array(
            'gameURL' => null, 
            'error' => $error_msg
        );
...
function WebhookPixGerado($nome_user, $url, $valor)
{
    ...
    // ID 1 é para Pixs Gerados e Pagos
    $queryWebhook = "SELECT * FROM webhook WHERE status = 1 AND id = 1";
...
function WebhookSaquesGerados($nome_user, $url, $valor)
{
    ...
    // ID 2 é para Saques
    $queryWebhook = "SELECT * FROM webhook WHERE status = 1 AND id = 2";
    $resultWebhook = mysqli_query($mysqli, $queryWebhook);
...
function WebhookCadastro($nome_user, $url, $email = '')
{
    ...
    // ID 3 é para Cadastros (caso você queira adicionar depois)
    $queryWebhook = "SELECT * FROM webhook WHERE status = 1 AND id = 3";
    $resultWebhook = mysqli_query($mysqli, $queryWebhook);
...
function adicionarSaldoUsuario($id_user, $valor) {
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt_log) {
            $tipo = "deposito_pix";
            $stmt_log->bind_param("idss", $id_user, $valor, $tipo, $data_registro);
            $stmt_log->execute();
