...
function ensureExpfyReferenceTable()
{
    global $mysqli;

    $sql = "CREATE TABLE IF NOT EXISTS gateway_expfypay_refs (
        id INT AUTO_INCREMENT PRIMARY KEY,
        external_id VARCHAR(128) NOT NULL UNIQUE,
        transacao_id VARCHAR(128) NOT NULL,
        usuario_id INT NOT NULL,
        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        KEY idx_transacao_id (transacao_id),
...
function salvarReferenciaExpfy($externalId, $transacaoId, $usuarioId)
{
    global $mysqli;

    ensureExpfyReferenceTable();

    $sql = "INSERT INTO gateway_expfypay_refs (external_id, transacao_id, usuario_id)
            VALUES (?, ?, ?)
    ...
    $stmt->bind_param("ssi", $externalId, $transacaoId, $usuarioId);
    $ok = $stmt->execute();
    $stmt->close();
...
function criarQrexpfypay($valor, $nome, $id, $comissao = null, $afiliado_id = null)
{
    ...
    try {
        $external_id = 'EXP_' . $id . '_' . bin2hex(random_bytes(8));
    } catch (Throwable $e) {
        $external_id = 'EXP_' . $id . '_' . uniqid();
    }
    ...
    if (isset($dados['success']) && $dados['success'] === true && isset($dados['data']['transaction_id'])) {
        ...
        if ($insert_paymentBD == 1) {
            salvarReferenciaExpfy((string)$external_id, (string)$transaction_id, (int)$id);
            $datapixreturn = [
                'transacao_id'   => $transaction_id,
                'transaction_id' => $transaction_id,
                'external_id'    => $dados['data']['external_id'] ?? $external_id,
