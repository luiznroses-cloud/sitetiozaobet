...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_cpa_individual'])) {
        ...
        if ($ok) {
            echo "<script>window.addEventListener('DOMContentLoaded', function() { showToast('success', 'CPA individual atualizado!'); });</script>";
            // Reload dados atuais
            $qry = "SELECT * FROM usuarios WHERE id='" . intval($id_user) . "'";
            $res = mysqli_query($mysqli, $qry);
            $data = mysqli_fetch_assoc($res);
            ...
            echo "<script>window.addEventListener('DOMContentLoaded', function() { showToast('danger', 'Erro ao atualizar CPA individual.'); });</script>";
        }
                        ...
                        <div class="tab-content">
                            <div class="tab-pane p-3" id="editar" role="tabpanel">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label for="lobby" class="form-label">Lobby Ativo?</label>
                                        <select id="lobby" name="lobby" class="form-select">
                                            <option value="1" <?= isset($data['lobby']) && $data['lobby'] == 1 ? 'selected' : '' ?>>Ativado</option>
                                            <option value="0" <?= isset($data['lobby']) && $data['lobby'] == 0 ? 'selected' : '' ?>>Desativado</option>
                                        </select>
                                    </div>
