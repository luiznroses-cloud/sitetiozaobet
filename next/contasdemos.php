...
// ==================== NOVA FUNÇÃO: ATUALIZAR SALDO EM MASSA ====================
function atualizarSaldoMassa($ids, $novo_saldo)
{
    global $mysqli;
    ...
    foreach ($ids as $id) {
        ...
        // Verifica se é conta demo
        $check_stmt = $mysqli->prepare("SELECT mobile, saldo FROM usuarios WHERE id = ? AND mobile LIKE 'demo%'");
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $stmt->bind_param("di", $novo_saldo, $id);
            ...
            if ($stmt->execute()) {
                ...
                $erros[] = "Erro ao atualizar conta $username (ID: $id): " . $stmt->error;
            }
            $stmt->close();
        } else {
            $erros[] = "Conta ID $id não é uma conta demo ou não existe";
        }
...
if (isset($_SESSION['resultado_atualizacao'])) {
    $resultado_atualizacao = $_SESSION['resultado_atualizacao'];
    unset($_SESSION['resultado_atualizacao']);
}

// PROCESSA ATUALIZAÇÃO DE SALDO EM MASSA
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['atualizar_saldo'])) {
    $ids = isset($_POST['contas_selecionadas']) ? $_POST['contas_selecionadas'] : [];
    $selecionar_todas = isset($_POST['selecionar_todas']) && $_POST['selecionar_todas'] === '1';
    $novo_saldo = floatval($_POST['novo_saldo']);

    // Quando "Selecionar Todas" estiver ativo, aplica em todas as contas demo do banco.
    if ($selecionar_todas) {
        $ids = [];
        $todos_ids_result = mysqli_query($mysqli, "SELECT id FROM usuarios WHERE mobile LIKE 'demo%'");
        while ($id_row = mysqli_fetch_assoc($todos_ids_result)) {
            $ids[] = $id_row['id'];
        }
    }
    
    if (!empty($ids) && $novo_saldo >= 0) {
        try {
            $resultado_atualizacao = atualizarSaldoMassa($ids, $novo_saldo);
            
            $_SESSION['resultado_atualizacao'] = $resultado_atualizacao;
            
            if ($resultado_atualizacao['sucesso'] > 0) {
                $_SESSION['toast_type'] = 'success';
                $_SESSION['toast_message'] = "{$resultado_atualizacao['sucesso']} contas atualizadas com sucesso!";
            } else {
                $_SESSION['toast_type'] = 'error';
                $_SESSION['toast_message'] = "Nenhuma conta foi atualizada";
            }
        } catch (Exception $e) {
            $_SESSION['toast_type'] = 'error';
            $_SESSION['toast_message'] = "Erro: " . $e->getMessage();
        }
    } else {
        $_SESSION['toast_type'] = 'error';
        $_SESSION['toast_message'] = "Selecione pelo menos uma conta e informe um saldo válido";
    }
...
$total_contas_demo = 0;
$total_contas_demo_qry = "SELECT COUNT(*) AS total FROM usuarios WHERE mobile LIKE 'demo%'";
$total_contas_demo_result = mysqli_query($mysqli, $total_contas_demo_qry);
if ($total_contas_demo_result && $total_row = mysqli_fetch_assoc($total_contas_demo_result)) {
    $total_contas_demo = intval($total_row['total']);
}

$contas_demo_qry = "SELECT * FROM usuarios WHERE mobile LIKE 'demo%' ORDER BY id DESC";
...
<body>
    ...
    <div class="page-wrapper">
        <div class="page-content">
            <div class="container-xxl">
                ...
                <!-- Resultado da Atualização de Saldo -->
                <?php if ($resultado_atualizacao && $resultado_atualizacao['sucesso'] > 0): ?>
                <div class="row justify-content-center mb-4">
                    <div class="col-lg-10">
                        <div class="card border-info">
                            <div class="card-header bg-info text-white">
                                <h4 class="card-title text-white mb-0">
                                    💰 Saldos Atualizados: <?= $resultado_atualizacao['sucesso'] ?>
                                </h4>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        ...
                                        <tbody>
                                            <?php foreach ($resultado_atualizacao['detalhes'] as $detalhe): ?>
                                            <tr>
                                                <td><?= $detalhe['id'] ?></td>
                                                <td><code><?= $detalhe['username'] ?></code></td>
                                                <td>R$ <?= number_format($detalhe['saldo_antigo'], 2, ',', '.') ?></td>
                                                <td><strong>R$ <?= number_format($detalhe['saldo_novo'], 2, ',', '.') ?></strong></td>
                                            </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                                    </table>
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-12">
                        <!-- Formulário de Atualização de Saldo (Sticky) -->
                        <div class="sticky-update-form">
                            <form method="POST" id="formAtualizarSaldo">
                                <input type="hidden" name="atualizar_saldo" value="1">
                                <input type="hidden" name="selecionar_todas" id="selecionarTodasHidden" value="0">
                                <div class="row align-items-end g-3">
                                    <div class="col-md-3">
                                        <label class="form-label text-white">
                                            <i class="fas fa-check-square"></i> 
                                            <span id="countSelecionadas">0</span> conta(s) selecionada(s)
                                        </label>
                                        ...
                                        <small class="text-muted">Aplicar em todas as contas demo cadastradas (<?= $total_contas_demo ?>)</small>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label text-white">Novo Saldo (R$)</label>
                                        <input type="number" name="novo_saldo" class="form-control" 
                                            step="0.01" min="0" value="1000.00" required>
                                    </div>
                                    <div class="col-md-5">
                                        <button type="submit" class="btn btn-warning w-100" id="btnAtualizar" disabled>
                                            <i class="fas fa-sync-alt"></i> Atualizar Saldo das Contas Selecionadas
                                        </button>
                                    </div>
                                </div>
                            </form>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Todas as Contas Demo (Total cadastrado: <?= $total_contas_demo ?>)</h4>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td colspan="8" class="text-center">Nenhuma conta demo encontrada</td>
                                            </tr>
                                            <?php else: ?>
                                            <?php foreach ($contas_demo as $conta): ?>
                                            <tr>
                                                <td>
                                                    <input type="checkbox" name="contas_selecionadas[]" 
                                                        value="<?= $conta['id'] ?>" 
                                                        class="form-check-input checkbox-conta"
                                                        form="formAtualizarSaldo">
                                                </td>
                                                <td><?= $conta['id'] ?></td>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const selecionarTodasHidden = document.getElementById('selecionarTodasHidden');
            const totalContasDemo = <?= intval($total_contas_demo) ?>;
            
            function atualizarContador() {
                const selecionadas = selectAll.checked
                    ? totalContasDemo
                    : document.querySelectorAll('.checkbox-conta:checked').length;
                countSelecionadas.textContent = selecionadas;
                btnAtualizar.disabled = selecionadas === 0;
            }
            
            selectAll.addEventListener('change', function() {
                checkboxes.forEach(cb => cb.checked = this.checked);
                selectAllTable.checked = this.checked;
                selecionarTodasHidden.value = this.checked ? '1' : '0';
                atualizarContador();
            });
            ...
            checkboxes.forEach(cb => {
                cb.addEventListener('change', function() {
                    ...
                    selectAllTable.checked = todasMarcadas;
                    selecionarTodasHidden.value = '0';
                    atualizarContador();
                });
            });
